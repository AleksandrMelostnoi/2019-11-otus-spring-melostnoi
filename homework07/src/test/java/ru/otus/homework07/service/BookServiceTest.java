...
public class BookServiceTest {
    ...
    void shouldCorrectSaveBook() throws EmptyFieldException {
        Author author = new Author(AUTHOR_ID, AUTHOR_NAME);
        Mockito.when(authorService.findByName(anyString())).thenReturn(author);
        Genre genre = new Genre(GENRE_ID, GENRE_NAME);
        Mockito.when(genreService.findByName(any())).thenReturn(genre);
        Book book = new Book(BOOK_NAME, genreService.findByName(GENRE_NAME), authorService.findByName(AUTHOR_NAME));
